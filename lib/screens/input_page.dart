...
class _PrecisionInputPageState extends State<PrecisionInputPage> {
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      // ▼▼▼ 強制オーバーライド: Center Count-Up (mode 0) の場合は、常に1に設定 ▼▼▼
      if (widget.gameMode == 0) {
        // Center Count-Upモードで起動した場合は、保存設定に関わらず
        // Inside Triple Ring (1) を強制適用
        boundaryType = 1;
      }
      // ▲▲▲ 修正ここから ▲▲▲
      ...
      switch (boundaryType) {
        case 0:
          _outBoundaryMm = 340.0; // Outside Double Ring
          break;
        case 1:
          _outBoundaryMm = 198.0; // Inside Triple Ring
          break;
