...
class _HistoryPageState extends State<HistoryPage> {
  ...
  Future<void> _openDetail(BuildContext context, Game game) async {
    ...
    final List<Offset> points = throwsData.map((t) => Offset(t.x, t.y)).toList();
            ...
            onPressed: () {
              if (_history.length < 2) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text("Play at least 2 games to see trends!")),
                );
                return;
              }
              Navigator.of(context).push(
      ...
      body: _history.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final dateStr = "${record.date.month}/${record.date.day} ${record.date.hour.toString().padLeft(2, '0')}:${record.date.minute.toString().padLeft(2, '0')}";
                ...
                return Card(
                  ...
                  child: ListTile(
                    leading: CircleAvatar(
                      ...
                      child: Text("${record.score}", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                    ),
                    title: Text(dateStr),
                    subtitle: Text("S:${record.ringSizeMm.toStringAsFixed(0)} / X:${record.sdX.toStringAsFixed(1)}"),
